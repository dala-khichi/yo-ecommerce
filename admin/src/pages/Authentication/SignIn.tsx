...
const handleFormData = (key,value)=>{
 ...
 const handleSubmit = async (e)=>{
  ...
  useEffect(() => {
    if (admin?.isLogin) {
      // If admin is logged in, try to go back
      if (location.key !== 'default') {
        navigate(-1); // Go back
      } else {
        navigate('/'); // If no history, go to home
      }
    }
    // If not logged in, do nothing (as per your requirement)
  }, [admin]); // Empty dependency array means this runs once on mount
