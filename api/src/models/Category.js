...
class Category {
    ...
    // Create new category
    static async create(img,{ name, slug, status, },adminId) {
      //console.log(name, slug, status, adminId ,img);
        const query = `
            INSERT INTO categories (name, slug, img, status, created_by)
            VALUES (?, ?, ?, ?, ?)
        `;
        const [result] = await db.execute(query, [
            name, slug, img, status, adminId
        ]);
        return result.insertId;
    }

    // Update category by ID
    static async update(id, img,{ name, slug, status, updated_by },adminId) {
      //log({name, slug, status, updated_by})
        ...
        await db.execute(query, [
            name, slug, img, status, adminId, id
        ]);
    }
    static async updateWithoutImg(id,{ name, slug, status, updated_by },adminId) {
        const query = `
            UPDATE categories
