...
class Item {
    ...
    // Create new item
    static async create({ name, description, category_id, subcategory_id, status },adminId) {
        const query = `
            INSERT INTO items (name, description, category_id, subcategory_id, status, created_by)
            VALUES (?, ?, ?, ?, ?, ?)
        `;
        const [result] = await db.execute(query, [name, description, category_id, subcategory_id, status, adminId]);
