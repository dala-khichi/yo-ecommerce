...
class Item {
  ...
  static async findAllV3(filters = {}) {
    try {
      let query = `
        SELECT i.*, 
               c.name as category_name, c.slug as category_slug,
  ...
  static async findAllV1(filters = {}) {
    try {
      let query = `
        SELECT 
               c.name as category_name, c.slug as category_slug,
      ...
      if (filters.tranding) {
        query =  `SELECT i.id,i.name,i.description,c.name as category_name, c.slug as category_slug,
               sc.name as subcategory_name, sc.slug as subcategory_slug
               FROM items i
  ...
  static async findAllV2(filters = {}) {
  try {
    let query = `
      SELECT i.*, 
             c.name as category_name, c.slug as category_slug,
    ...
    // ✅ Sirf ek image aur variants hata diye
    const itemsWithDetails = await Promise.all(
      items.map(async (item) => {
        const [images] = await db.query(
          'SELECT * FROM item_images WHERE item_id = ? LIMIT 1',
          [item.id]
        );
        return {
          ...item,
          image: images[0] || null   // ✅ single image object
        };
      })
    );
  ...
  static async findById(id) {
    try {
      ...
      const item = items[0];
      const [images] = await db.query('SELECT img FROM item_images WHERE item_id = ?', [id]);
      const [variants] = await db.query('SELECT iv.stock,iv.price,iv.id,c.color, s.size FROM item_variants iv  JOIN colors c ON  iv.color_id  = c.id JOIN  sizes s ON iv.size_id= s.id    WHERE item_id = ?', [id]);
//stock,price,id,c.color, s.size
      return {
        ...item,
        images:images.map((e,i)=>e.img),
        variants
      };
    } catch (error) {
